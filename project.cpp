...
std::vector<double> FCFS(std::map<char, process*> proc_map, std::vector<process*> procs, int t_cs)
{
...
/*##################################################################*/
			...
			if(cpu->rem_burst_time != 0)
			{
				...
				if(cpu->rem_burst_time == 0)
				{
					...
					else
					{
						...
						{
							std::cout << "time " << time << "ms: Process " << cpu->curr_proc->id << " completed a CPU burst; " << cpu->curr_proc->rem_bursts << " burst to go ";
							printq(readyQ);
						}
					}
	...
	for(int i = 0; i < procs.size(); i++)
	{
		...
		for(int j = 0; j < procs[i]->cpu_bursts.size(); j++)
		{
...
void print_stats(char alg[], double ave_cpu_bt, std::vector<double> data, FILE* out)
{
	double x = 0;
...
void start_sim(int n_procs, int seed, double lambda, int upper, int t_cs, double alpha, int t_slice, FILE* out, std::string rr_add, std::string algorithm)
{
	...
	if(algorithm == "FCFS")
	{
		data = RR(proc_map, procs, t_cs, 9999999, "END", "FCFS");
		char alg1[] = {'F', 'C', 'F', 'S'};
		print_stats(alg1, ave_cpu_bt, data, out);
		std::cout << "\n";
	}
		/*
		data = SJF(proc_map, procs, t_cs);
		char alg2[] = {'S', 'J', 'F'};
		print_stats(alg2, ave_cpu_bt, data, out);

		data = SRT(proc_map, procs, t_cs);
		char alg3[] = {'S', 'R', 'T'};
		print_stats(alg3, ave_cpu_bt, data, out);
		*/
	else
	{
		data = RR(proc_map, procs, t_cs, t_slice, rr_add, "RR");
		char alg4[] = {'R', 'R'};
		print_stats(alg4, ave_cpu_bt, data, out);
	}
}	
